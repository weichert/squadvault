...
p = Path("scripts/check_no_memory_reads.sh")
s = p.read_text()

# Replace the entire script with a deterministic allowlist-based check.
# (bash3-safe; grep-only; repo-root independent)
new = r'''#!/usr/bin/env bash
set -euo pipefail
