...
# Match a *bare* split string literal: " <newline> " (or single-quote variant).
# We avoid touching triple-quote constructs by requiring exactly one quote on each side.
BARE_SPLIT_DQUOTE = re.compile(r'(?<!")"\s*\n\s*"(?!")')
BARE_SPLIT_SQUOTE = re.compile(r"(?<!')'\s*\n\s*'(?!')")

def _apply(text: str) -> tuple[str, int]:
    changed = 0

    text2, n1 = BARE_SPLIT_DQUOTE.subn(r'"\\n"', text)
    changed += n1

    text3, n2 = BARE_SPLIT_SQUOTE.subn(r"'\\n'", text2)
    changed += n2
...
def main() -> None:
    ...
    for p in OFFENDERS:
        ...
        if edits:
            ...
            print(f"OK: {p} (no bare split-quote tokens detected)")
