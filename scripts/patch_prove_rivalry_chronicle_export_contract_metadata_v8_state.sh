#!/usr/bin/env bash
set -euo pipefail
echo "=== Patch: Rivalry Chronicle export contract add State: APPROVED (v8) ==="
...
$PY scripts/_patch_prove_rivalry_chronicle_export_contract_metadata_v8_state.py

echo "==> bash syntax check"
bash -n scripts/prove_rivalry_chronicle_end_to_end_v1.sh

echo "==> show window around metadata vals + upserts"
