...
def main() -> int:
    if not GATE.exists():
        raise SystemExit(f"REFUSE: missing expected gate: {GATE}")

    current = GATE.read_text(encoding="utf-8")
    if current == NEW_TEXT:
        return 0

    # We keep this strict: only allow replacement if the file looks like our gate.
    if "idempotence allowlist wrappers must not recurse into prove_ci" not in current:
        raise SystemExit(f"REFUSE: {GATE} does not look like expected gate; won't overwrite")
