...
DOC_ANCHOR = "This index enumerates **active, enforced CI guardrails** for the SquadVault ingest repo."
...
def ensure_file(path: Path, content: str) -> None:
    if path.exists():
        ...
        if existing == content:
            return
        # Fail fast rather than overwrite unexpected drift.
        raise SystemExit(f"ERROR: {path} already exists with different content")
...
def patch_doc() -> None:
    ...
    if DOC_ANCHOR not in text:
        raise SystemExit("ERROR: doc anchor not found; CI_Guardrails_Index structure drifted")

    text = text.replace(DOC_ANCHOR, DOC_ANCHOR + "\n\n" + DOC_INSERT.strip() + "\n", 1)
    DOC.write_text(text, encoding="utf-8")

def main() -> int:
    ensure_file(HELPER, HELPER_TEXT)
    patch_doc()
    print(f"Patched/added: {HELPER}")
    print(f"Patched: {DOC}")
    return 0
