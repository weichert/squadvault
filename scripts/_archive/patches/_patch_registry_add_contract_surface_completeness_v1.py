...
BLOCK = f"""\
{BEGIN}
- scripts/gate_contract_surface_completeness_v1.sh — Gate: contract surface completeness (v1)
  ...
  if anchor in s:
    ...
    for line in lines:
      ...
      if (not inserted) and (anchor in line):
        ...
        inserted = True
    if not inserted:
      raise SystemExit("ERROR: failed to insert bounded block at anchor (line scan).")
