...
PROVE_INSERT_AFTER = "bash scripts/gate_ci_guardrails_ops_label_source_exactness_v1.sh"
INDEX_INSERT_AFTER = "scripts/gate_ci_guardrails_ops_label_source_exactness_v1.sh"
TSV_INSERT_AFTER = "scripts/gate_ci_guardrails_ops_label_source_exactness_v1.sh"
...
def patch_index() -> None:
    ...
    if not replaced:
        inserted = False
        for i, line in enumerate(lines):
            if INDEX_INSERT_AFTER in line:
                lines.insert(i + 1, TARGET_BULLET)
                inserted = True
                break
        if not inserted:
...
def patch_tsv() -> None:
    ...
    replaced = False
    for i, line in enumerate(lines):
        if line.startswith(TARGET_SCRIPT + "\t"):
            lines[i] = TARGET_TSV_LINE
            replaced = True
            break
    ...
    if not replaced:
        inserted = False
        for i, line in enumerate(lines):
            if line.startswith(TSV_INSERT_AFTER + "\t"):
                lines.insert(i + 1, TARGET_TSV_LINE)
                inserted = True
                break
        if not inserted:
