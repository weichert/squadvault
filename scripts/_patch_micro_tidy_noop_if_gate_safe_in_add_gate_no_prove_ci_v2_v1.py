...
TARGET = REPO_ROOT / "scripts" / "_patch_add_gate_idempotence_allowlist_wrappers_no_prove_ci_v2.py"

REQUIRED_MARKER = "GATE_TEXT_BASH32_SAFE"
GATE_NAME = "gate_idempotence_allowlist_wrappers_no_prove_ci_v1.sh"
...
def main() -> int:
    if not TARGET.exists():
        raise SystemExit(f"REFUSE: missing target to patch: {TARGET}")

    old = TARGET.read_text(encoding="utf-8")

    if REQUIRED_MARKER not in old or GATE_NAME not in old:
        raise SystemExit(f"REFUSE: {TARGET} does not look like expected v2 patcher; won't overwrite")
