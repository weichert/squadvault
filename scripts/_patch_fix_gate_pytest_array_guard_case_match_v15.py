...
# Anchor: the violation append line (we replace guard immediately before it).
VIOL_RE = re.compile(
    r'^\s*violations\+\="\$\{file\}:\$\{lineno\}: pytest target must start with Tests/ \(found '
)
...
def main() -> int:
    ...
    # Find the first violation append line after marker (bounded)
    try:
        v_idx = next(
            i for i in range(m_idx + 1, min(len(lines), m_idx + 200))
            if VIOL_RE.match(lines[i])
        )
    except StopIteration:
        raise SystemExit("ERROR: could not find violation append line after marker (refusing to guess).")

    # Replace [marker .. just before violation] with canonical guard block
    new_lines: list[str] = []
    new_lines.extend(lines[:m_idx])
    new_lines.extend(GUARD_BLOCK)
    new_lines.extend(lines[v_idx:])

    new_s = "".join(new_lines)
    if new_s == s:
        _chmod_x(p)
        print("OK: pytest array-expansion guard already canonical (noop).")
        return 0

    _clipwrite(REL, new_s)
    _chmod_x(p)
    print("OK: rewrote pytest array-expansion guard to safe case-match (v15).")
    return 0
