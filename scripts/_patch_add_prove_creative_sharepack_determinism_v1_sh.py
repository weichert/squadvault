...
TARGET = Path("scripts/prove_creative_sharepack_determinism_v1.sh")
...
clean_sharepack
SV_LEAGUE_ID="${league_id}" SV_SEASON="${season}" SV_WEEK_INDEX="${week_index}" \
  ./scripts/py scripts/gen_creative_sharepack_v1.py --league-id "${league_id}" --season "${season}" --week-index "${week_index}"

echo "==> [2] Gate #1"
SV_LEAGUE_ID="${league_id}" SV_SEASON="${season}" SV_WEEK_INDEX="${week_index}" \
  bash scripts/gate_creative_sharepack_output_contract_v1.sh
...
SV_LEAGUE_ID="${league_id}" SV_SEASON="${season}" SV_WEEK_INDEX="${week_index}" \
  "${repo_root}/scripts/py" "${repo_root}/scripts/gen_creative_sharepack_v1.py" --league-id "${league_id}" --season "${season}" --week-index "${week_index}"

echo "==> [5] Gate #2"
SV_LEAGUE_ID="${league_id}" SV_SEASON="${season}" SV_WEEK_INDEX="${week_index}" \
  bash "${repo_root}/scripts/gate_creative_sharepack_output_contract_v1.sh"
...
def main() -> None:
    ...
    TARGET.write_text(PROOF_TEXT, encoding="utf-8", newline="\n")
    print("OK")
