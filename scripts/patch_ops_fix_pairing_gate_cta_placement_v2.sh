...
echo "=== Patch: fix pairing gate CTA placement (v2) ==="
...
./scripts/py scripts/_patch_ops_fix_pairing_gate_cta_placement_v2.py

echo "==> bash syntax check (spot)"
bash -n scripts/check_patch_pairs_v1.sh
bash -n scripts/patch_ops_fix_pairing_gate_cta_placement_v2.sh

echo "==> smoke: force failure (tracked unpaired patcher) and confirm ordering"
