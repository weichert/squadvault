...
PATCH_ADD = REPO / "scripts" / "_patch_add_gate_ci_guardrails_ops_label_registry_parity_v1.py"
WRAP_ADD = REPO / "scripts" / "patch_add_gate_ci_guardrails_ops_label_registry_parity_v1.sh"
HELPER = REPO / "scripts" / "_patch_fix_ops_label_registry_parity_wiring_v2.py"
...
TARGET_TSV = f"{TARGET_SCRIPT}\t{TARGET_DESC}"
...
def extract_cluster_lines(text: str) -> list[str]:
    started = False
    out = []
...
cluster_scripts = [line.replace("bash ", "", 1) for line in extract_cluster_lines(prove_text)]
...
for script_path in sorted(set(cluster_scripts) - set(tsv_rows)):
    errors.append(f"Ops cluster script missing from TSV registry: {script_path}")

for script_path in sorted(set(cluster_scripts) - set(index_rows)):
