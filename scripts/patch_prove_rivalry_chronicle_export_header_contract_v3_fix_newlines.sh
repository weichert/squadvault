#!/usr/bin/env bash
set -euo pipefail
echo "=== Patch: fix Rivalry Chronicle export header contract newline literals (v3) ==="
...
$PY scripts/_patch_prove_rivalry_chronicle_export_header_contract_v3_fix_newlines.py

echo "==> bash syntax check"
bash -n scripts/prove_rivalry_chronicle_end_to_end_v1.sh

echo "==> show window around the contract join/write"
