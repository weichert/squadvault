...
# - executable wrappers in scripts/
...
files() {
  ...
  find scripts -maxdepth 1 -type f -name "recap.py" -print
  # any other top-level scripts without extension that are meant to be run (e.g. scripts/py, scripts/recap.sh already covered)
  find scripts -maxdepth 1 -type f ! -name "*.*" -print
}

is_excluded() {
  case "$1" in
    scripts/check_shims_compliance.sh) return 0 ;;
    scripts/_patch_*.py) return 0 ;;
  esac
  return 1
}
...
if [[ "${FIX}" == "1" ]]; then
  ...
  ./scripts/py scripts/_patch_operator_scripts_use_shims_v1.py || true
  ./scripts/py scripts/_patch_examples_use_shims_v1.py || true
  echo
fi
...
while IFS= read -r line; do
  [[ -z "$line" ]] && continue
  echo "WARN: hardcoded python path found (consider replacing with scripts/py): $line" >&2
