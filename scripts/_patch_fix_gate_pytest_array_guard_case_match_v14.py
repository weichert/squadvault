...
def main() -> int:
    ...
    # Find violation line after marker (must exist; bound search to avoid guessing)
    try:
        v_idx = next(i for i in range(m_idx, min(len(lines), m_idx + 120)) if VIOL_RE.match(lines[i]))
    except StopIteration:
        raise SystemExit("ERROR: could not find violation append line after marker (refusing to guess).")

    # Find end of current marker block: first line exactly 'fi' between marker and violation
    try:
    ...
    if new_s == s:
        _chmod_x(p)
        print("OK: pytest array-expansion guard already case-matching canonical (noop).")
        return 0

    _clipwrite(REL, new_s)
    _chmod_x(p)
    print("OK: rewrote pytest array-expansion guard to case-match normalized token (v14).")
    return 0
