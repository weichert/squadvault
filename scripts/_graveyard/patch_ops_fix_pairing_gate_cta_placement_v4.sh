...
echo "=== Patch: dedupe + place pairing CTA after allowlist line (v4) ==="
...
"$PY" scripts/_patch_ops_fix_pairing_gate_cta_placement_v4.py

echo "==> bash syntax check (spot)"
bash -n scripts/check_patch_pairs_v1.sh
bash -n scripts/patch_ops_fix_pairing_gate_cta_placement_v4.sh
