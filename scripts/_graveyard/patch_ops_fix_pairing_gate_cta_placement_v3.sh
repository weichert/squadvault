...
echo "=== Patch: dedupe + reposition pairing gate CTA (v3) ==="
...
"$PY" scripts/_patch_ops_fix_pairing_gate_cta_placement_v3.py

echo "==> bash syntax check (spot)"
bash -n scripts/check_patch_pairs_v1.sh
bash -n scripts/patch_ops_fix_pairing_gate_cta_placement_v3.sh
