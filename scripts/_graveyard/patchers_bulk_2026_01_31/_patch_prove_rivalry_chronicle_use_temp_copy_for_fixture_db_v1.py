...
def main() -> None:
    ...
    if MARK in s:
        print("OK: Rivalry Chronicle prove patch already applied.")
        return

    lines = s.splitlines()

    # Insert after first db= assignment (most scripts parse args into db=...)
    insert_idx = None
    ...
    # Fallback: insert after cd "${REPO_ROOT}" if present, else after shebang
    ...
    TARGET.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
    print("OK: patched prove_rivalry_chronicle_end_to_end_v1.sh to use temp db when db is fixtures/*.")
