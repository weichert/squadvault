...
def strip_markers(txt: str) -> str:
    ...
    for line in txt.splitlines(True):
        s = line.lstrip()
        # comment-only marker lines (our gate only keys off comment form, but strip either way)
        if s.startswith("#") and any(p in s for p in PREFIXES):
            # drop any line that contains either marker key
            continue
