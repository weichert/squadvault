...
TARGET = Path("src/squadvault/consumers/rivalry_chronicle_generate_v1.py")
...
def parse_top_module_docstring(lines: list[str], start_idx: int) -> tuple[int | None, int | None]:
  ...
  if line.startswith('"""') or line.startswith("'''"):
    ...
    # Multi-line docstring: find closing delimiter
...
def header_scan_start(lines: list[str]) -> int:
  """
  Skip shebang + encoding lines only. Returns index of first line after those.
  """
  i = 0
...
def first_nontrivia(lines: list[str], start: int) -> int:
  """
  First line index >= start that is not blank and not comment.
  """
  i = start
...
def first_code_statement(lines: list[str]) -> int:
  """
  Returns index of first executable statement (not blank/comment/shebang/encoding/docstring/import).
  If no such statement, returns len(lines).
  """
...
def ensure_markers() -> None:
  ...
  # If markers exist but appear after executable code, refuse
