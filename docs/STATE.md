...
- L.1 FIRST WAVE SCOPE RULED 2026-06-19 (DECIDE; verified engine `3c39eb6` / frontend `d298a2a`, both main; brief anchors matched, branch sweep main-only). Ruling memo `_observations/OBSERVATIONS_2026_06_19_L1_FIRST_WAVE_SCOPE_RULING.md`. Minimal slice = CAPTURE-ONLY: `member_history_sessions` (founding_sessions sibling, member-identity-keyed via franchise_member_links NOT flat franchise_id; append-only RLS, member-only INSERT); one member end-to-end; testimony attributed + unmerged; display DEFERS. PAYLOAD = the two-layer separation (testimony provably cannot be read as / merged into an event fact; fails-closed no-ledger-write probe = G22 analogue). D-L1-1..6 ratified as recommended. MFI = FOLDED not predecessor (testimony is a distinct + SAFER class than MFI's human-attested-EVENT-fact class; MFI D1-D6 stay OPEN on their own track; NON-CROSSING: a remembered datum is never auto-promoted to the event ledger). Consent = NEW dedicated category `oral_history_testimony` (017 idiom, member-only INSERT, no rendering_class, founder-applied SQL editor) for revocation granularity over attributed_quotes; audio (recorded_voice) DEFERS to L.4. Sweep = pre-draft REMOTE window, member-only consent (commissioner cannot proxy, W.6 1.3), per-member not all-ten-gated, 08-15 soft target; L.1 stands as own surface riding E2.3 auth+outreach. W.8 = REGISTER-only (section-9 consumer in L.1 spec). STANDING: member_history_sessions migration + oral_history_testimony CHECK-widen need a FRESH prod object-existence probe before apply (EXECUTE). Docket item 3 (L.1) -> IN-FLIGHT; next = L.1 SPECIFICATION (Fable, twelve-section per-surface template). Behind it: W.1 Inc 2.
