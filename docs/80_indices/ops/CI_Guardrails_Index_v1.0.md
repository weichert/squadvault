...
- scripts/gate_ci_proof_runners_block_sorted_v1.sh — Prove Ops index contains proof-surface registry discoverability marker + bullet (v1)
...
- scripts/gate_ci_prove_ci_relative_script_invocations_v1.sh — Contracts: manifest hash exactness gate (v1)
- scripts/gate_ci_registry_execution_alignment_v1.sh — Enforce docs/contracts/README.md indexes all versioned contracts (v1)
- scripts/gate_ci_runtime_envelope_v1.sh — Enforce creative surface fingerprint artifact canonical (v1)
- scripts/gate_contract_surface_manifest_hash_v1.sh — Creative surface fingerprint canonical gate (v1)
- scripts/gate_contracts_index_discoverability_v1.sh — Creative Surface Registry parity gate (v1)
- scripts/gate_creative_surface_fingerprint_canonical_v1.sh — CI guardrails gate: enforce Creative Surface registry usage (v1)
- scripts/gate_creative_surface_fingerprint_v1.sh — CWD independence (shims) gate (v1)
- scripts/gate_creative_surface_registry_discoverability_v1.sh — Docs integrity gate: enforce canonical docs invariants (v2)
- scripts/gate_creative_surface_registry_parity_v1.sh — Guardrail: proofs must not mutate docs unexpectedly (v2)
- scripts/gate_creative_surface_registry_usage_v1.sh — Enforce CI uses temp working DB copy (fixture immutable) (v1)
- scripts/gate_cwd_independence_shims_v1.sh — Meta: surface parity aggregator gate (v1)
- scripts/gate_docs_integrity_v2.sh — Disallow bare '==>' marker lines in scripts/*.sh (v1)
- scripts/gate_docs_mutation_guardrail_v2.sh — Disallow 'scripts/scripts/' path invocations (v2)
- scripts/gate_enforce_test_db_routing_v1.sh — Forbid network/package-manager actions in CI proof surfaces (v1)
- scripts/gate_meta_surface_parity_v1.sh — Reject obsolete allowlist rewrite recovery artifacts (v1)
- scripts/gate_no_bare_chevron_markers_v1.sh — Detect pasted terminal banner content in scripts/ (v1)
- scripts/gate_no_double_scripts_prefix_v2.sh — Enforce no test dir case drift (Tests/ vs tests/) (v1)
- scripts/gate_no_mapfile_readarray_in_scripts_v1.sh — Guardrail: fail if untracked patch artifacts exist (v1)
- scripts/gate_no_network_in_ci_proofs_v1.sh — No-xtrace guardrail gate (v1)
- scripts/gate_no_obsolete_allowlist_rewrite_artifacts_v1.sh — Enforce Ops indices contain no autofill placeholders (v1)
- scripts/gate_no_terminal_banner_paste_v1.sh — Enforce patch-wrapper allowlist is canonical + safe (v1)
- scripts/gate_no_test_dir_case_drift_v1.sh — Enforce proof runners match registry exactly (v1)
- scripts/gate_no_untracked_patch_artifacts_v1.sh — Gate vs proof boundary: enforce Proof Surface Registry excludes scripts/gate_*.sh (v1)
- scripts/gate_no_xtrace_v1.sh — Pytest must only target tracked Tests/ paths (v1)
- scripts/gate_ops_indices_no_autofill_placeholders_v1.sh — Rivalry Chronicle contract doc linkage gate (v1)
