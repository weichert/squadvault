...
- scripts/gate_no_xtrace_v1.sh — no forbidden 'set -x' found in prove/gate scripts.
- scripts/gate_cwd_independence_shims_v1.sh — CWD-independence gate passed.
- scripts/gate_allowlist_patchers_must_insert_sorted_v1.sh — allowlist patchers must insert-sorted (no append-before-EOF) (v1)
- scripts/gate_ci_guardrails_ops_entrypoint_parity_v1.sh — CI Guardrails ops entrypoints parity (v1)
- scripts/gate_ci_guardrails_ops_entrypoint_exactness_v1.sh — CI Guardrails ops entrypoints exactness (v1)
- scripts/gate_worktree_cleanliness_v1.sh — Gate FAIL: worktree cleanliness (v1) ===" >&2
...
- scripts/gate_ci_proof_runners_block_sorted_v1.sh — CI Proof Runners Block Sorted (v1)
- scripts/gate_ci_proof_surface_registry_exactness_v1.sh — proof surface registry exactness gate (v1)
...
- scripts/gate_ci_prove_ci_relative_script_invocations_v1.sh — CI Prove CI Relative Script Invocations (v1)
- scripts/gate_ci_registry_execution_alignment_v1.sh — CI registry → execution alignment (v1)
- scripts/gate_ci_runtime_envelope_v1.sh — CI Runtime Envelope (v1)
- scripts/gate_contract_surface_manifest_hash_v1.sh — Contract Surface Manifest Hash (v1)
- scripts/gate_contracts_index_discoverability_v1.sh — Contracts Index Discoverability (v1)
- scripts/gate_creative_surface_fingerprint_canonical_v1.sh — Creative surface fingerprint canonical (v1)
- scripts/gate_creative_surface_fingerprint_v1.sh — Creative Surface Fingerprint (v1)
- scripts/gate_creative_surface_registry_discoverability_v1.sh — Creative Surface Registry Discoverability (v1)
- scripts/gate_creative_surface_registry_parity_v1.sh — creative surface registry parity gate passed (v1)
- scripts/gate_creative_surface_registry_usage_v1.sh — Creative Surface Registry Usage (v1)
...
- scripts/gate_meta_surface_parity_v1.sh — Meta Surface Parity (v1)
- scripts/gate_no_bare_chevron_markers_v1.sh — no bare '==>' marker lines in scripts/*.sh (v1)
- scripts/gate_no_double_scripts_prefix_v2.sh — no double scripts prefix (v2)
- scripts/gate_no_mapfile_readarray_in_scripts_v1.sh — No Mapfile Readarray In Scripts (v1)
- scripts/gate_no_network_in_ci_proofs_v1.sh — No network/package-manager actions in CI proofs (v1)
- scripts/gate_no_obsolete_allowlist_rewrite_artifacts_v1.sh — no obsolete allowlist rewrite recovery artifacts (v1)
- scripts/gate_no_terminal_banner_paste_v1.sh — No terminal banner paste gate
- scripts/gate_no_test_dir_case_drift_v1.sh — No Test Dir Case Drift (v1)
- scripts/gate_no_untracked_patch_artifacts_v1.sh — No Untracked Patch Artifacts (v1)
- scripts/gate_ops_indices_no_autofill_placeholders_v1.sh — Ops indices must not contain autofill placeholders (v1)
- scripts/gate_patch_wrapper_idempotence_allowlist_v1.sh — patch wrapper idempotence (allowlist) v1
- scripts/gate_proof_suite_completeness_v1.sh — proof suite completeness (v1)
- scripts/gate_proof_surface_registry_excludes_gates_v1.sh — Proof Surface Registry Excludes Gates (v1)
- scripts/gate_prove_ci_structure_canonical_v1.sh — \s*.*===["'\''"]\s*$' "$PROVE" > "$banners_file" || true
- scripts/gate_pytest_tracked_tests_only_v1.sh — Pytest Tracked Tests Only (v1)
- scripts/gate_repo_clean_after_proofs_v1.sh — Repo Clean After Proofs (v1)
- scripts/gate_rivalry_chronicle_contract_linkage_v1.sh — Rivalry Chronicle Contract Linkage (v1)
- scripts/gate_standard_show_input_need_coverage_v1.sh — Standard Show Input Need Coverage (v1)
...
<!-- SV_CREATIVE_SURFACE_REGISTRY_DISCOVERABILITY_v1_BEGIN -->
<!-- SV_CREATIVE_SURFACE_REGISTRY: v1 -->
- docs/80_indices/ops/Creative_Surface_Registry_v1.0.md — Creative Surface Registry (canonical pointers) (v1)
<!-- SV_CREATIVE_SURFACE_REGISTRY_DISCOVERABILITY_v1_END -->
