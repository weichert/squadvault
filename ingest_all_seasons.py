...
def _mfl_json(url: str, timeout: int = 30, label: str = "") -> Dict[str, Any]:
    """Fetch JSON from MFL API with 429 backoff."""
    for attempt in range(1, 6):
        try:
            ...
            if e.code == 429:
                wait = 30 * attempt
                print(f"    {label or 'fetch'}: 429 rate limited, waiting {wait}s (attempt {attempt}/5)")
                time.sleep(wait)
                continue
            raise
...
def ingest_matchups(
    *,
    ...
    Uses urllib directly with aggressive 429 backoff (30s) to handle
    MFL rate limiting reliably.
    """
    host = f"www{mfl_server}.myfantasyleague.com"
    ...
    for week in range(1, max_weeks + 1):
        url = f"https://{host}/{season}/export?TYPE=weeklyResults&L={mfl_league_id}&JSON=1&W={week}"
        ...
        for attempt in range(1, 6):  # up to 5 attempts
            try:
                ...
                if e.code == 429:
                    wait = 30 * attempt
                    print(f"    W{week}: 429 rate limited, waiting {wait}s (attempt {attempt}/5)")
                    time.sleep(wait)
                    continue
                else:
                    break  # non-429 HTTP error = skip this week
            except Exception:
                break  # network error = skip this week
...
def ingest_transactions(
    *,
    ...
    Uses urllib directly with aggressive 429 backoff.
    """
    host = f"www{mfl_server}.myfantasyleague.com"
    url = f"https://{host}/{season}/export?TYPE=transactions&L={mfl_league_id}&JSON=1"

    store = SQLiteStore(db_path)

    # Pause before fetching — respect rate limits
    time.sleep(5.0)
    ...
    for attempt in range(1, 6):
        try:
            ...
            if e.code == 429:
                wait = 30 * attempt
                print(f"    transactions: 429 rate limited, waiting {wait}s (attempt {attempt}/5)")
                time.sleep(wait)
                continue
            else:
                print(f"    FETCH ERROR: HTTP {e.code}")
                return 0, 0
        except Exception as e:
            print(f"    FETCH ERROR: {e}")
            return 0, 0
    ...
    if raw_json is None:
        print(f"    FETCH FAILED after retries")
        return 0, 0
...
def ingest_season(
    *,
    ...
    # Pause between seasons to respect MFL rate limits
    time.sleep(5.0)
...
def main() -> int:
    ...
    ap.add_argument("--end-year", type=int, default=2025, help="Last season (default: 2025)")
