...
> **Note 2026-04-18:** The "11 detectors scoped but not built" framing immediately above is superseded. All 19 player narrative detectors across Dimensions 1–5 are implemented and wired in `src/squadvault/core/recaps/context/player_narrative_angles_v1.py`; this state was first formally verified in commit `8af867f` (2026-04-09). The trade-loader functional bug documented in that same commit was resolved in commit `39c8f1c` the same day. The S10-pattern architectural leak in `_load_season_trades` remains open and is inventoried in `OBSERVATIONS_2026_04_18_S10_SCOPE_CORRECTION.md`. The general argument about detectors as the resolution to the silence-vs-delight tension is unaffected.
